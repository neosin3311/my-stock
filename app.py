...
# API 호출 주기는 4초로 유지하되, 깜빡임 속도를 위해 0.5초마다 데이터를 다시 그립니다.
...
# 하나의 신호로 전 종목의 색상을 결정 (동시 깜빡임의 핵심)
...
with placeholder.container():
    for name, info in st.session_state.my_stocks.items():
        ...
        if is_up:
            ...
            color = "#f04452"
            # 기준 충족 시 백엔드 변수 타이밍(blink_state)에 맞춰 클래스 지정
...
# 0.5초마다 코드를 강제로 돌려 실시간으로 동일하게 온/오프 처리
time.sleep(0.5)
st.rerun()
